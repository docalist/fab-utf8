...
function diff_xindex(diff, loc) {
  ...
  var last_chars2 = 0;
  for (var x=0; x<diff.length; x++) {
    if (diff[x][0] != DIFF_INSERT) // Equality or deletion.
      chars1 += diff[x][1].length;
    if (diff[x][0] != DIFF_DELETE) // Equality or insertion.
  ...
  if (diff.length != x && diff[x][0] == DIFF_DELETE) // The location was deleted.
    return last_chars2;
...
function diff_prettyhtml(diff) {
  ...
  var html = '';
  for (var x=0; x<diff.length; x++) {
    var m = diff[x][0]; // Mode (delete, equal, insert)
...
function patch_make(text1, text2, diff) {
  ...
  for (var x=0; x<diff.length; x++) {
    ...
    } else if (diff_type == DIFF_EQUAL && diff_text.length <= 2*PATCH_MARGIN && patch.diffs.length != 0 && diff.length != x+1) {
    ...
    last_type = diff_type;
    if (diff_type == DIFF_EQUAL && diff_text.length >= 2*PATCH_MARGIN) {
